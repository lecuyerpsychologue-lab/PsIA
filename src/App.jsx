...
const callAPI = async (mode, data={}, context={}, history=[]) => {
  try {
    const res = await fetch('/api/chat', { 
      method: 'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify({mode, data, context, history}) 
    });
...
const SOS = ({ close }) => (
  <div className="fixed inset-0 z-50 bg-white/95 flex items-center justify-center p-6 animate-fade">
...
const Echo = ({ exit, saveNote }) => {
    ...
    const handleSave = () => {
        saveNote(`Bilan Météo ÉCHO`, `Météo : ${aiRes.weather.text}\n\nAnalyse : ${aiRes.analysis}`, 'echo');
        alert('Sauvegardé dans le Journal !');
    };
...
const Hero = ({ exit, saveNote }) => {
    ...
    const handleSave = () => {
        ...
        alert('Programme sauvegardé dans le Journal !');
    };
...
const Identity = ({ exit, onDone, saveNote }) => {
    ...
    const handleSave = () => {
        saveNote(`Mon Profil - ${res.archetype}`, `Portrait : "${res.text}"\n\nDétails :\n${Object.entries(ans).map(([k,v])=>`- ${k}: ${v}`).join('\n')}`, 'identity');
